...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        elif module_def['type'] == 'vit':
            vit = ViT_BACKBONE(
                width=int(module_def['width']),
                height=int(module_def['height']),
                patch_size=int(module_def['patch_size']),
                dim=int(module_def['dim']),
                depth=int(module_def['depth']),
                heads=int(module_def['heads']),
                mlp_dim=int(module_def['mlp_dim']),
                dropout=float(module_def['dropout']),
                emb_dropout=float(module_def['emb_dropout'])
            )
...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, targets_len=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool', 'vit']:
                x = module(x)
            elif mtype == 'route':
